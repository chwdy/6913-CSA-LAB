...
public:
  ...
  INSMem()
  {
    ...
    string line;
    int i = 0;
    imem.open("imem.txt"); // imem.open("imem2.txt");
    if (imem.is_open())
    {
  ...
  bitset<32> ReadMemory(bitset<32> ReadAddress)
  {
    ...
    printf("Read INS memory at addr:" ); 
	cout << addrptr.to_ulong()<< endl;
...
//  int NextPC;
  ...
  while (1)
  {
	...
	bitset<32> 	NextPC = bitset<32> (PC.to_ulong()+4);; //TODO
	
	concat = bitset<32>(bitset<4>(bitadd(PC,bitset<32>(4)).to_string().substr(0,4)).to_string()+strins.substr(6, 26)+"00");         //4bit
    //After RF
    adder = bitset<32>(
      signextimm(bitset<16>(strins.substr(16,16))
        ).to_string().substr(2,30)
       +"00");
       
       
    adder = bitset<32>(adder.to_ulong()+NextPC.to_ulong());
       
    isEq =  (myRF.ReadData1 == myRF.ReadData2); //
    //(myRF.ReadData1.to_ulong()==myRF.ReadData2.to_ulong())?1:0;
